...
## #11a. Л6 — ТУННЕЛЬ, БЕТОННЫЙ (ранняя часть, S22a–S23c)

Нужен, чтобы морф в кадре 24 читался. Без контраста бетон → камень самый
дорогой приём фильма превращается в ничто.

```
Cinematic film still, wide shot from the driver's point of view inside a car,
looking straight down the INTERIOR of an ordinary modern road tunnel, the
edge of a dark dashboard and steering wheel along the bottom of the frame,
perfectly centred symmetrical composition, the tunnel receding far into
darkness, a long row of orange sodium lamps along the ceiling, lane markings
running away from the camera, walls of SMOOTH POURED CONCRETE with visible
form-work seams and flat grey panels on both sides, plain utilitarian
construction, no other cars, headlights raking the near walls, amber orange
fading into coal black, claustrophobic, 24mm lens, cinematic film still,
35mm film grain, natural motivated lighting, shallow depth of field, subtle
halation, 16:9
```
...
НЕГАТИВНЫЙ ПРОМПТ:
```
no rock, no stone walls, no cave, no natural rock texture, no veins in the wall,
no exterior view, no sky, no daylight, no other vehicles, no people
```

## #11c. Л6 — ВЪЕЗД В ТУННЕЛЬ, СНАРУЖИ (для кадра 21)
...
|---|---|---|---|
| 1 | Шохрух, лицо | ПРИНЯТ — РЕФЕРЕНС-ПАНЕЛЬ ГЕРОЯ. Единственный источник ШРАМА на левой брови: карта персонажа его не удержала, поэтому к каждому ECU лица прикрепляются ОБА — карта и этот эталон | шрам на левой брови виден, тени под глазами, неровная щетина, миндалевидные тёмно-карие. Подставляется во ВСЕ панели с героем |
...
| 11a | Л6 туннель, БЕТОННЫЙ (ранний) | НУЖЕН | гладкий поворённый бетон, швы опалубки. Работает в S22a–S23c и начале S24a |
| 11b | Л6 туннель, КАМЕННЫЙ (поздний) | ПРИНЯТ | вид изнутри из-за руля, центральная симметрия, точка схода, грубая скала с прожилками, тьма в конце. Работает в S24c–S26c |
...
**КАРТА ШОХРУХА #C1 — ПРИНЯТА.** Turnaround (фронт, профиль, спина) +
expression sheet на 6 состояний: нейтраль, паника, страх с поднятыми
ладонями, изумление, достоинство, тихая улыбка. Часы на левом запястье,
ботинки коричневая кожа. НЕ ПОПАЛО: шрам на левой брови и ракурс 3/4.
Шрам берётся из эталона #1, 3/4 генератор достроит между фронтом и профилем.

**ДВА ТУННЕЛЯ ВМЕСТО ОДНОГО.** Первая удачная генерация интерьера дала
СРАЗУ КАМЕННЫЕ стены. Красиво и достоверно, но на превращении бетона в
камень построен кадр 24 — морф без склейки, единственный момент, где
зритель ВИДИТ смену мира до степи. Если туннель каменный с самого начала,
замечать нечего. Поэтому нужны два эталона: 11a бетонный для ранней части
и 11b каменный для поздней.

**Итого: 14 принято. Осталось 5: Темур в шатре (4a), Темур перед войском (4b), шатёр (14), ТУННЕЛЬ БЕТОННЫЙ (11a), сквер сверху (17).**
